...
class HashTable:
    ...
    def put(self, key, value):
        ...
        bucket_index = calculate_hash(key) & (self.bucket_size - 1)
    ...
    def judge_rehash_expand(self):
        if self.item_count > self.bucket_size * 0.7:
            return True
        else:
            return False
        
    def judge_rehash_shrink(self):
        if self.item_count < self.bucket_size * 0.3 and self.rehashed == True and self.bucket_size > 100:
            return True
        else:
            return False
    ...
    # Set the table size externally.
    def set_bucket_size(self, size):
        self.bucket_size = size
        return

    # Rehash the hash table if the buckets exceeds 70%.
    # Extend the bucket size by 2 times.
    def rehash_expand(self):
        prev_bucket_size = self.bucket_size
        prev_item_count = self.item_count
        self.bucket_size <<= 1
        self.buckets += [None] * prev_bucket_size
        for bucket_index in range(prev_bucket_size):
            item = self.buckets[bucket_index]
            while item:
                temp_key, temp_value = item.key, item.value
                temp_index = calculate_hash(temp_key) & (prev_bucket_size - 1)
                item = item.next
                self.delete(temp_key, temp_index)
                self.put(temp_key, temp_value)
        assert (self.item_count == prev_item_count)
        self.rehashed = True
        return

    # Rehash the hash table if the buckets becomes lower than 30%.
    # Reduce the size by half.
    def rehash_shrink(self):
        self.bucket_size >>= 1
        for bucket_index in range(self.bucket_size):
            item = self.buckets[bucket_index]
            if item == None:
                self.buckets[bucket_index] = self.buckets[bucket_index + self.bucket_size]
                continue
            while item.next != None:
                item = item.next
            item.next = self.buckets[bucket_index + self.bucket_size]
        return
