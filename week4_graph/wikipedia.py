...
class Wikipedia:
    ...
    def check_links(self):
        for i in self.links.keys():
            if len(self.links[i]) == 0 and self.links[i] == []:
                print(i)

    # Calculate the page ranks and print the most popular pages.
    def find_most_popular_pages(self):
        num_of_pages = len(self.titles)
        # num_of_pages = 1188228
        Google_matrix = np.zeros((num_of_pages, num_of_pages))
        for i in range(1, num_of_pages+1):
            if i not in self.links or self.links[i] == []:
                continue
            prob = 0.85 / len(self.links[i])
            for link in self.links[i]:
                Google_matrix[link-1, i-1] = prob
        Google_matrix = Google_matrix + np.ones((num_of_pages, num_of_pages)) / num_of_pages * 0.15

        pagerank = np.ones(num_of_pages)
        while True:
            pagerank_prev = pagerank * 1.0
            pagerank = Google_matrix @ pagerank
            assert(abs(pagerank.sum()-pagerank_prev.sum()) < 1e-10)
            if np.linalg.norm(pagerank - pagerank_prev) < 1e-8:
                break

        pagerank_list = []
        for i in range(num_of_pages):
            pagerank_list.append([i+1, pagerank[i]])
        pagerank_list.sort(key=lambda x: x[1], reverse=True)
        # print(pagerank_list)
        print("Page rank ranking")
        for rank in range(10):
            print("%d: %s" % (rank+1, self.titles[pagerank_list[rank][0]]))
